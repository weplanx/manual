...
如果有更高的安全需要可以将以下环境变量和 ConfigMap 变更为 Secret 再挂载引入
...
定义 `default.values.yml`，配置名即动态配置的 **Snake Case**，例如：
...
如果 XAPI 必须要经过外网，可以使用 ipWhiteList 与 basicAuth 中间件，例如
